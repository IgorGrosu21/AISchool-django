...
class DetailedMediaView(generics.CreateAPIView, generics.DestroyAPIView):
  ...
  def get_container(self) -> WithFiles:
    pass
  ...
  def post(self, request: Request, *args, **kwargs):
    ...
    try:
      ...
      raise ValueError(f"File type not allowed: {file.content_type}") from e

    try:
      validate_file_size(file, MAX_FILE_SIZE)
    except ValueError as e:
      raise ValueError(f"File too large: {file.size} bytes (max: {MAX_FILE_SIZE})") from e
